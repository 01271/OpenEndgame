		...
		try {
			...
			if (!$openid->mode) {
				...
				if ($openid->validate()) {
					...
					$_SESSION['sid'] = (int) $matches[1];//This is where the user's steamID is set, IMPORTANT.
...
class LightOpenID {
	 ...
	 *
	 * @param unknown $url url to check
	 ...
	 *
	 * @param unknown $url Identity URL.
	...
	function discover($url) {
		...
		// We'll jump a maximum of 2 times, to avoid endless redirections.
